...
export function toPriorLessonContext(lesson: StoredLesson): PriorLessonContext {
  const miniLesson =
    lesson.output.sections?.["Mini Lesson"] ?? lesson.output.teachingMove ?? "";
  return {
    date: new Date(lesson.timestamp).toISOString().slice(0, 10),
    goals:
      lesson.output.reasoning?.instructional_goals.map((g) => g.goal) ?? [
        lesson.output.needSummary,
      ],
    targetPatterns:
      lesson.output.reasoning?.lesson_constraints.target_phonics_patterns ??
      lesson.output.targetWords,
    miniLessonExcerpt:
...
function isStoredLesson(obj: unknown): obj is StoredLesson {
  ...
  return (
    ...
    !!o.output &&
    typeof (o.output as LessonOutput).needSummary === "string"
  );
}
