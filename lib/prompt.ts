...
export function buildUserPrompt(input: TeacherInput): string {
  return [
    `Student name: ${input.studentName}`,
    `Grade: ${input.grade}`,
    `Strengths the teacher has observed: ${input.strengths}`,
    `Struggles the teacher has observed: ${input.struggles}`,
