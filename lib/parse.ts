...
export function parseStrict(raw: string): LessonOutput | null {
  ...
  return { reasoning, sections: sections as Record<SectionHeader, string> };
}
...
export function validateLesson(output: LessonOutput): string[] {
  ...
  const story = output.sections["Decodable Story"];
  if (!story) {
  ...
  const targetWords = extractTargetWords(
    output.reasoning.lesson_constraints.target_phonics_patterns,
  );
